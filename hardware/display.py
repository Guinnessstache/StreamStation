...
Supports:
  - TM1637 4-digit 7-segment display (Pi 5 compatible, no external library)
  - I2C LCD 16x2 / 20x4 via RPLCD (older Pi models)

Pi 5 note: Uses custom tm1637_driver.py instead of the 'tm1637' PyPI package,
which does not support the Pi 5 /proc/cpuinfo format.
"""
...
class TM1637Display:
    """
    4-digit 7-segment display via TM1637.
    Uses bundled tm1637_driver.py — no PyPI library required.
    ...
    def __init__(self, clk_pin, dio_pin, brightness=3):
        try:
            ...
            from tm1637_driver import TM1637
            self.display   = TM1637(clk=clk_pin, dio=dio_pin, brightness=brightness)
            self.available = True
            log.info(f"TM1637 initialized — CLK=GPIO{clk_pin}, DIO=GPIO{dio_pin}")
        except ImportError as e:
            log.warning(f"RPi.GPIO not available: {e}")
    ...
    def show_channel(self, number):
        ...
        try:
            ...
            log.warning(f"TM1637 show_channel error: {e}")
    ...
    def show_dashes(self):
        ...
        try:
            ...
            log.warning(f"TM1637 dashes error: {e}")

    def show_text(self, text):
        if not self.available:
            return
        try:
            self.display.show(list(text[:4].upper()))
        except Exception as e:
            log.warning(f"TM1637 show_text error: {e}")


# ── I2C LCD Driver ─────────────────────────────────────────────────────────────
class LCDDisplay:
    """
    I2C character LCD (16x2 or 20x4) via RPLCD.
    For older Pi models with standard I2C on bus 1.
    """

    def __init__(self, i2c_address=0x27, cols=16, rows=2, bus=1):
        try:
            ...
            self.available = True
            self.lcd.clear()
            log.info(f"LCD initialized — I2C 0x{i2c_address:02X}, bus {bus}, {cols}x{rows}")
        except ImportError:
...
class DisplayManager:
    def __init__(self, config):
        ...
        if display_type in ("lcd", "both"):
            ...
            bus  = hw.get("lcd_bus", 1)
            self.lcd = LCDDisplay(addr, cols, rows, bus)
    ...
    def update(self, channel, name, state):
        ...
        if channel and state == "playing":
            ...
            if self.lcd:
                self.lcd.show_dashes()

    def run(self, poll_interval=1.0):
        log.info("Display driver running")
        while True:
            try:
